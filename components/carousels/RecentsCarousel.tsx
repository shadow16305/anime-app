...
const RecentCarousel: React.FC<RecentCarouselProps> = ({ items }) => {
  return (
    <>
      <h2 className="mt-24 ps-4 text-[32px] font-bold text-white">Recent</h2>
      <Carousel className="relative mt-8 px-4" opts={{ align: "start" }}>
        <CarouselContent>
          {items.map((item) => (
            <CarouselItem
              key={item.id}
